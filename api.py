...
def _parse_track(item: dict) -> dict | None:
    ...
    for field in img_fields:
        ...
        if field == "album" and isinstance(img_data, dict): img_data = img_data.get("images", [])
...
def get_playlist_sync(playlist_id: str):
    ...
    # Aumentamos o timeout para 30s para evitar ReadTimeout no Render
    res = httpx.get(url, headers=_EMBED_HEADERS, follow_redirects=True, timeout=30.0)
    if res.status_code != 200: raise Exception(f"Spotify retornou {res.status_code}")
    soup = BeautifulSoup(res.text, "html.parser")
    script = soup.find("script", {"id": "resource"}) or soup.find("script", {"id": "__NEXT_DATA__"})
    if not script or not script.string: raise Exception("Dados não encontrados no HTML")
...
def scrape_spotify_generator(url: str, access_token: str):
    ...
    try:
        ...
        if sp:
            ...
            # Tentativa ultra-básica da API para evitar o 403 de filtros complexos
            ...
            limit = 100
            while True:
                # Usamos playlist_tracks que é o endpoint mais antigo e estável
